...
router.get('/public/demo/:token', async (req, res) => {
    try {
        ...
        // Set VERY permissive CSP headers - demos need to load any external content
...
// POST contact from demo landing page (NO AUTH)
router.post('/demos/contact', async (req, res) => {
    try {
        ...
        await db.query(
            `INSERT INTO hunter_demo_contact_requests (prospect_id, name, email, phone, message)
             VALUES ($1, $2, $3, $4, $5)`,
            [prospectId, name, email, phone || null, message || null]
        );
...
// Aplicar middleware a todas las rutas Hunter autenticadas
